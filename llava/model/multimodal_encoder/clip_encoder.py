...
class CLIPVisionTower(nn.Module):
    ...
    def token_prune_merge_advanced_training(self, images, if_adaptive=True, reduction_ratio = 1/8):
        '''
        version 10/03/2024 using the key*key matrix to calculate the cosine similarity
        '''
        # token_indix_list = []
        ...
        if if_adaptive:
            for i in range(B):
                image_feature_single = image_features[i].unsqueeze(0)
                desired_layer_k_single = desired_layer_k[i].unsqueeze(0)
                desired_layer_q_single = desired_layer_q[i].unsqueeze(0)
                attn = (desired_layer_q_single @ desired_layer_k_single.transpose(-2, -1)) * C ** -0.5
                attn = F.softmax(attn, dim=-1)
                cls_attn = attn[:, 0, 1:]
            ...
            # reduction_ratio = outlier_dectection(cls_attn)#*3.5
                ...
                Key_wo_cls = desired_layer_k_single[:, 1:]  # [B, N-1, C]

                x_others = torch.gather(image_feature_single, dim=1, index=index)  # [B, left_tokens, C]
                x_others_attn = torch.gather(cls_attn, dim=1, index=idx)  
                Key_others = torch.gather(Key_wo_cls, dim=1, index=index)  # [B, left_tokens, C]
                compl = complement_idx(idx, N)  # [B, N-1-left_tokens]
                non_topk = torch.gather(image_feature_single, dim=1, index=compl.unsqueeze(-1).expand(-1, -1, C))  # [B, N-1-left_tokens, C]
                non_topk_Key = torch.gather(Key_wo_cls, dim=1, index=compl.unsqueeze(-1).expand(-1, -1, C))
                non_topk_attn = torch.gather(cls_attn, dim=1, index=compl)  # [B, N-1-left_tokens]
                ...
                sub_B, left_tokens, _ = x_others.size()
                updated_x_others = torch.zeros_like(x_others)
                ...
                for b in range(sub_B):
                    for i in range(left_tokens):
                        ...
                        _, cluster_indices = torch.topk(cos_sim_matrix, k=int(16), dim=2, largest=True)
                ...
                extra_one_token = torch.sum(non_topk * non_topk_attn.unsqueeze(-1), dim=1, keepdim=True)  # [B, 1, C]
                updated_x_others = torch.cat([updated_x_others, extra_one_token],dim=1)
                image_feature_single = updated_x_others
                image_features_list.append(image_feature_single) 
    ...
    def token_prune_merge_advanced_plus_training(self, images, if_adaptive=True, reduction_ratio = 1/8):
        '''
        version 02/04/2024 using the spacially smapled tokens to supplement the pruned tokens
        return a feature tensor list with the different shape as the input image tensor
        '''
        # token_indix_list = []
        # token_indix_dict = {}
        image_features_list = []
        ...
        new_idx = torch.zeros((idx.size(0), idx.size(1)*2), dtype=torch.long).to(device=self.device)
        for i in range(idx.size(0)):
            arithmetic_sequence = torch.arange(int(step_length/3), 575, int(step_length)).to(device=self.device)
            original_tensor_1d = idx[i].flatten().to(device=self.device)
            filtered_sequence = arithmetic_sequence
            # filtered_sequence = torch.tensor([x for x in arithmetic_sequence if x not in original_tensor_1d]).to(device=self.device)
            concatenated_tensor = torch.cat((original_tensor_1d, filtered_sequence), dim=0)
        ...
        for b in range(B):
            ...
            image_features_list.append(updated_center)
        # extra_one_token = torch.sum(non_topk * non_topk_attn.unsqueeze(-1), dim=1, keepdim=True)  # [B, 1, C]
        # updated_x_others = torch.cat([updated_x_others, extra_one_token],dim=1)
        # image_features = updated_x_others
        return image_features_list
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            # image_features = self.token_prune_merge_advanced_training(images, if_adaptive=True, reduction_ratio=1/8)
            
            # image_features = self.token_prune_merge_advanced_plus(images, if_adaptive=False, reduction_ratio=1/8)
            # image_features = self.token_prune_merge_advanced_plus_training(images, if_adaptive=True, reduction_ratio=1/8)
